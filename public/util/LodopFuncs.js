...
function loadCLodop() {
    ...
    if (window.location.protocol=='https:') {
      ...
      JS1.src = "http://localhost:8009/CLodopfuncs.js";
 ...
 function getLodop(oOBJECT, oEMBED) {
    ...
    try {
        ...
        if (needCLodop()) {
            ...
            if (!LODOP) {
                window.LODOP_ERROR_MESSAGE = strCLodopInstall_1 + (CLodopIsLocal ? strCLodopInstall_2 : "") + strCLodopInstall_3;
                return;
            } else {
                if (CLODOP.CVERSION < "4.1.4.2") {
                    window.LODOP_ERROR_MESSAGE = strCLodopUpdate + window.LODOP_ERROR_MESSAGE;
                }
            ...
            if ((!LODOP) || (!LODOP.VERSION)) {
                if (ua.indexOf('Chrome') >= 0)
                    window.LODOP_ERROR_MESSAGE = strHtmChrome + window.LODOP_ERROR_MESSAGE;
                if (ua.indexOf('Firefox') >= 0)
                    window.LODOP_ERROR_MESSAGE = strHtmFireFox + window.LODOP_ERROR_MESSAGE;
                window.LODOP_ERROR_MESSAGE = (is64IE ? strHtm64_Install : strHtmInstall) + window.LODOP_ERROR_MESSAGE;
                return LODOP;
            }
        }
        if (LODOP.VERSION < "6.2.2.6") {
            if (!needCLodop())
                window.LODOP_ERROR_MESSAGE = (is64IE ? strHtm64_Update : strHtmUpdate) + window.LODOP_ERROR_MESSAGE;
        }
